...
CREATE OR REPLACE VIEW `{{ project_id_tgt }}.{{ dataset_reporting_tgt_sfdc }}.RecordTypesMD`
  OPTIONS(description = 'View for Record Types table')
AS (
  SELECT
  ...
  FROM `{{ project_id_src }}.{{ dataset_cdc_processed_sfdc }}.record_types` AS RecordTypes
);
