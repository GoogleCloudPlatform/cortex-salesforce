...
CREATE OR REPLACE VIEW `{{ project_id_tgt }}.{{ dataset_reporting_tgt_sfdc }}.UsersMD`
  OPTIONS(description = 'View for User Table')
AS (
  SELECT
    Users.UserId,
    Users.AboutMe,
    ...
    Users.UserName
  FROM
    `{{ project_id_src }}.{{ dataset_cdc_processed_sfdc }}.users` AS Users
);
